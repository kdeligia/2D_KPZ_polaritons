...
def theta_data(i_batch, p, sigma, gamma0, gamma2, g, mypath):
    for i_n in range(n_internal):
        ...
        theta = gpe.time_evolution_theta(dt = dt, N_input = N_input, i_start = i_start, di = di)
        np.savetxt(mypath + os.sep + 'trajectories' + '_' + 'core' + str(i_batch + 1) + '_' + str(i_n + 1) + '.dat', theta)
        if (i_n + 1) % 2 == 0:
