...
# =============================================================================
    def time_evolution(self):
        ...
        psi_correlation_x = ext.isotropic_avg(self.psi_x, self.psi_x[center_indices[0][0], center_indices[0][1]], 'psi correlation', **isotropic_indices)
        n_avg_x = ext.isotropic_avg(self.n(self.psi_x), None, 'density average', **isotropic_indices)
        return psi_correlation_x, n_avg_x, psi_correlation_t, n_avg_t
...
def names_subfolders(sigma_array, p_array):
    save_folder = path + os.sep + 'TEST_ns' + str(int(ns_tilde)) + '_' + 'g' + str(g_dim)+ '_' + 'gr' + str(gr_dim) + '_' + 'gamma' + str(gamma0_tilde)
...
def g1_separate(i_batch, p, sigma, om_tilde, g_dim, gr_dim):
        ...
        np.save(subfolders_spatial['p=' + str(p), 'sigma=' + str(sigma)] + os.sep + 'correlation_spatial' + '_' +'core' + str(i_batch + 1) + '.npy', g1_x_batch)
        np.save(subfolders_spatial['p=' + str(p), 'sigma=' + str(sigma)] + os.sep + 'avg_density_spatial' + '_' + 'core' + str(i_batch + 1) + '.npy', avg_dens_x_batch)
        np.save(subfolders_temporal['p=' + str(p), 'sigma=' + str(sigma)] + os.sep + 'correlation_temporal' + '_' +'core' + str(i_batch + 1) + '.npy', g1_t_batch)
...
def g1_full(i_batch, p, sigma, om_tilde, g_dim, gr_dim):
        ...
        np.save(subfolders_full['p=' + str(p), 'sigma=' + str(sigma)] + os.sep + 'correlation' + '_' +'core' + str(i_batch + 1) + '.npy', g1_full_batch)
        np.save(subfolders_full['p=' + str(p), 'sigma=' + str(sigma)] + os.sep + 'avg_density' + '_' + 'core' + str(i_batch + 1) + '.npy', avg_dens_full_batch)
        return None
